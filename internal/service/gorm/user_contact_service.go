...
import (
	...
	"go-chat/pkg/enum/contact_apply/contact_apply_status_enum"
...
// GetUserList 获取用户列表
// 查到的是所有联系人，如果被禁用或者被拉黑会以弹窗的形式提醒，无法打开会话框，如果被删除，无法找到该联系人
...
func (u *userContactService) GetContactInfo(contactId string) (string, respond.GetContactInfoRespond, int) {
	...
	if contactId[0] == 'G' {
		...
		if group.Status != group_status_enum.DISABLE {
			...
			zlog.Error("该群聊已被禁用")
			return "该群聊已被禁用", respond.GetContactInfoRespond{}, -1
		}
	} else { // 非群聊
		var user model.UserInfo
		...
		if user.Status != user_status_enum.DISABLE {
			...
			zlog.Error("该用户已被禁用")
			return "该用户已被禁用", respond.GetContactInfoRespond{}, -1
		}
	}
}

// DeleteContact 删除联系人(用户)
func (u *userContactService) DeleteContact(ownerId string, contactId string) (string, int) {
	...
	// 双向删除会话
	if res := dao.GormDB.Model(&model.Session{}).Where("send_id = ? OR receive_id = ?", ownerId, contactId).Update("deleted_at", deletedAt); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
	}

	if res := dao.GormDB.Model(&model.ContactApply{}).Where("user_id = ? AND contact_id = ?", contactId, ownerId).Update("deleted_at", deletedAt); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
	}
	...
	if err := myredis.DelKeysWithSuffix("my_contact_list_" + ownerId); err != nil {
		zlog.Error(err.Error())
	}
		...
		if res := dao.GormDB.Where("user_id = ? AND contact_id = ?", req.OwnerId, req.ContactId).First(&contactApply); res.Error != nil {
			...
			if errors.Is(res.Error, gorm.ErrRecordNotFound) {
				contactApply = model.ContactApply{
					...
					Status:      contact_apply_status_enum.PENDING, // 申请待通过的状态
					Message:     req.Message,
...
// GetNewContactList 获取新的联系人申请列表 即 待处理的好友请求
func (u *userContactService) GetNewContactList(ownerId string) (string, []respond.NewContactListRespond, int) {
	...
	if res := dao.GormDB.Where("contact_id = ? AND status = ?", ownerId, contact_apply_status_enum.PENDING).Find(&contactApplyList); res.Error != nil {
		if errors.Is(res.Error, gorm.ErrRecordNotFound) {
			zlog.Info("没有待处理的好友请求")
			return "没有待处理的好友请求", nil, 0
		} else {
			zlog.Error(res.Error.Error())
			return constants.SYSTEM_ERROR, nil, -1
		}
...
// GetAddGroupList 获取加群申请列表
// 只有群主群管理员才能调用这个接口 前端先判断是
...
func (u *userContactService) PassContactApply(ownerId string, contactId string) (string, int) { // ownerId同意contactId发起的加好友/加群申请
	// ownerId 如果是用户的话就是用户账号，如果是群聊的话就是群聊id
	var contactApply model.ContactApply
	if res := dao.GormDB.Where("contact_id = ? AND uuid = ?", ownerId, contactId).First(&contactApply); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
	}
	...
	if ownerId[0] == 'U' { // 用户
		var user model.UserInfo
		if res := dao.GormDB.Where("uuid = ?", ownerId, contactId).First(&user); res.Error != nil {
			zlog.Error(res.Error.Error())
		}
		if user.Status == user_status_enum.DISABLE { // 用户被禁用
			zlog.Error("用户被禁用")
			return "用户被禁用", -2
		}

		// 通过联系人请求
		contactApply.Status = contact_apply_status_enum.ACCEPT
		if res := dao.GormDB.Save(&contactApply); res.Error != nil {
			zlog.Error(res.Error.Error())
			return constants.SYSTEM_ERROR, -1
		}
		...
		if group.Status == group_status_enum.DISABLE { // 群聊被禁用
			zlog.Error("群聊被禁用")
			return "群聊被禁用", -2
		}
		...
		// 将通过申请的用户添加到目标群组的成员列表中，更新群组成员列表，并删除该群组的缓存以保证数据最新
		var members []string
		// 解析群组的成员JSON字符串
...
func (u *userContactService) RefuseContactApply(ownerId string, contactId string) (string, int) { // ownerId拒绝contactId发起的加好友/加群申请
	var contactApply model.ContactApply
	if res := dao.GormDB.Where("contact_id = ? AND uuid = ?", ownerId, contactId).First(&contactApply); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
...
func (u *userContactService) CancelBlackContact(ownerId string, contact string) (string, int) {
	// 判断ownerId和contactId是否是拉黑和被拉黑
	var blackContact model.UserContact
	if res := dao.GormDB.Where("user_id = ? AND contact_id = ?", ownerId, contact).First(&blackContact); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
	}
	if blackContact.Status != contact_status_enum.BLACK && blackContact.Status != contact_status_enum.BE_BLACK {
		return "未拉黑该联系人，无需解除拉黑", -2
	}

	var beBlackContact model.UserContact
	if res := dao.GormDB.Where("user_id = ? AND contact_id = ?", contact, ownerId).First(&beBlackContact); res.Error != nil {
		zlog.Error(res.Error.Error())
		return constants.SYSTEM_ERROR, -1
	}

	if beBlackContact.Status != contact_status_enum.BLACK && beBlackContact.Status != contact_status_enum.BE_BLACK {
