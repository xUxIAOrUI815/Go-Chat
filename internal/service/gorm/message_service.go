...
func (m *messageService) GetMessageList(userOneId, userTwoId string) (string, []respond.GetMessageListRespond, int) {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) { // 如果缓存没查到
			zlog.Info(err.Error())
			zlog.Info(fmt.Sprintf("%s %s", userOneId, userTwoId))
			var messageList []model.Message
			if res := dao.GormDB.Where("(send_id = ? AND receive_id = ?) OR (send_id = ? AND receive_id = ?)", userOneId, userTwoId, userTwoId, userOneId).Find(&messageList); res.Error != nil {
...
// GetGroupMessageList 获取群聊聊天记录
func (m *messageService) GetGroupMessageList(groupId string) (string, []respond.GetGroupMessageListRespond, int) {
	rspString, err := myredis.GetKeyNilIsErr("group_message_list_" + groupId)
	if err != nil {
		if errors.Is(err, redis.Nil) { // 如果在redis中没有找到
			var messageList []model.Message
